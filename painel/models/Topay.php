...
class Topay extends Model{
	...
	public function add( $id_company, $id_user, $description, $maturity, $n_pmt, $pmt_n,  $price ){
		//echo $id_company.' '.$id_user.' '.$description.' '.$maturity.' '.$n_pmt.' '.$pmt_n.' '.$price;exit;
		$id_provider=0;
		$sql="INSERT INTO topay SET id_company = :id_company, id_user = :id_user, description = :description, maturity = :maturity, n_pmt = :n_pmt, pmt_n = :pmt_n, price = :price, id_provider = :id_provider, date_topay= NOW() ";
	...
	public function getInfoById($id, $id_company){
		...
		$sql = $this->db->prepare("SELECT * FROM topay WHERE id_company=:id_company AND id=:id ");
		$sql->bindValue(":id_company", $id_company);
		$sql->bindValue(":id", $id);
		$sql->execute();
